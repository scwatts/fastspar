...
void printHelp() {
    std::cerr << "Program: FastSpar exact p-values (c++ implementation of SparCC)" << std::endl;
    std::cerr << "Version 0.0.2" << std::endl;
    std::cerr << "Contact: Stephen Watts (s.watts2@student.unimelb.edu.au)" << std::endl;
    std::cerr << std::endl;
    std::cerr << "Usage:" << std::endl;
    std::cerr << "  exact_pavlues [options] --otu_table <of> --correlation <rf> --prefix <p> --permutations <n> --output <o>" << std::endl;
    std::cerr << std::endl;
    std::cerr << "  -c/--otu_table <of>   OTU input table used to generated correlations" << std::endl;
    std::cerr << "  -r/--correlation <rf> Correlation table generated by FastSpar" << std::endl;
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        ...
        std::cerr << ": error: options -c/--otu_table, -r/--correlation, -p/--prefix, -n/--permutations and -o/--outfile are required" << std::endl;
        exit(0);
    }
    ...
    while (1) {
        int option_index = 0;
        int c;
        c = getopt_long (argc, argv, "hc:p:r:n:o:t:", long_options, &option_index);
        if (c == -1) {
            break;
        }
        switch(c) {
