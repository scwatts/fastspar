C  This was file downloaded from http://www.netlib.org/go/gaussq.f
C  on 7 August 2012.
C  Two lines have been commented out, with a capital C, and rewritten
C  by Gordon Smyth.  The file is unchanged apart from those two changes
C  and this preamble of 6 lines.
...
c
C      double precision b(n), t(n), w(n), endpts(2), muzero, t1,
C     x gam, solve, dsqrt, alpha, beta
c
C      call class (kind, n, alpha, beta, b, t, muzero)
c
...
c           submatrix.
c
C      if (kpts.eq.0)  go to 100
C      if (kpts.eq.2)  go to  50
c
c           if kpts=1, only t(n) must be changed
c
C      t(n) = solve(endpts(1), n, t, b)*b(n-1)**2 + endpts(1)
C      go to 100
c
c           if kpts=2, t(n) and b(n-1) must be recomputed
c
C   50 gam = solve(endpts(1), n, t, b)
C      t1 = ((endpts(1) - endpts(2))/(solve(endpts(2), n, t, b) - gam))
C      b(n-1) = dsqrt(t1)
C      t(n) = endpts(1) + gam*t1
c
...
C  105    w(i) = 0.0d0
c
C      call gausq2 (n, t, b, w, ierr)
C      do 110 i = 1, n
C  110    w(i) = muzero * w(i) * w(i)
c
C      return
C      end
c
c
c
C      double precision function solve(shift, n, a, b)
c
...
c
c
C      double precision shift, a(n), b(n), alpha
c
C      alpha = a(1) - shift
C      nm1 = n - 1
C      do 10 i = 2, nm1
C   10    alpha = a(i) - shift - b(i-1)**2/alpha
C      solve = 1.0d0/alpha
C      return
C      end
c
...
c
C      double precision a(n), b(n), muzero, alpha, beta
...
C         a(i) = 0.0d0
C         abi = i
C   11    b(i) = abi/dsqrt(4*abi*abi - 1.0d0)
C      a(n) = 0.0d0
C      return
c
...
c
C   40 muzero = dsqrt(pi)
C      do 41 i = 1, nm1
C         a(i) = 0.0d0
C   41    b(i) = dsqrt(i/2.0d0)
C      a(n) = 0.0d0
C      return
c
...
c
c
      subroutine gausq2(n, d, e, z, ierr) bind (c, name='c_gausq2')
c
