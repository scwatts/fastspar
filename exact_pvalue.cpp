...
void printHelp() {
    ...
    std::cerr << "  exact_pavlues --otu_table <of> --correlation <rf> --prefix <p> --permutations <n>" << std::endl;
    std::cerr << std::endl;
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        ...
        std::cerr << ": error: options -t/--otu_table, -r/--correlation, -p/--prefix, and -n/--permutations are required" << std::endl;
        exit(0);
    }
    ...
    while (1) {
        int option_index = 0;
        int c;
        c = getopt_long (argc, argv, "ht:p:r:n:", long_options, &option_index);
        if (c == -1) {
            break;
        }
        switch(c) {
